...
def make_biased_dataloader(args):
    ...
    source_subject_test.append(source_subject_train.pop(1)) #p015
...
def source_imsave(source_img_train,source_label_train,source_img_test,source_label_test):
    ...
    for count in range(len(source_img_train)):
        if not os.path.exists(path+'source/train/'+source_label_train[count][1]):
            os.makedirs(path+'source/train/'+source_label_train[count][1])
        shutil.copy(path+source_img_train[count], path+'source/train/'+source_label_train[count][1])
    
    for count in range(len(source_img_test)):
        if not os.path.exists(path+'source/test/'+source_label_test[count][1]):
            os.makedirs(path+'source/test/'+source_label_test[count][1])
        shutil.copy(path+source_img_test[count], path+'source/test/'+source_label_test[count][1])
...
def target_imsave(target_img_train,target_label_train,target_img_test,target_label_test):
    ...
    for count in range(len(target_img_train)):
        if not os.path.exists(path+'target/train/'+target_label_train[count][1]):
            os.makedirs(path+'target/train/'+target_label_train[count][1])
        shutil.copy(path+target_img_train[count], path+'target/train/'+target_label_train[count][1])
    
    for count in range(len(target_img_test)):
        if not os.path.exists(path+'target/test/'+target_label_test[count][1]):
            os.makedirs(path+'target/test/'+target_label_test[count][1])
        shutil.copy(path+target_img_test[count], path+'target/test/'+target_label_test[count][1])
